...
class Economy:
        ...
        if self.bank.account_exists(author):
            ...
            if "Propayday" in roles:
                if id in self.payday_register[server.id]:
                    ...
                    if seconds >= self.settings[server.id]["PRO_TIME"]:
                        if "LeGeNDary™" in roles:
                            self.bank.deposit_credits(author, self.settings[
                                server.id]["LEGENDARY_CREDITS"])
                        ...
                        dtime = self.display_time(
                            self.settings[server.id]["PRO_TIME"] - seconds)
                        await self.bot.say(
    ...
    @economyset.command(pass_context=True)
    async def propaydaytime(self, ctx, seconds: int):
        """Seconds between each payday"""
        server = ctx.message.server
        self.settings[server.id]["PRO_TIME"] = seconds
        await self.bot.say("Value modified. At least {} seconds must pass "
    ...
    @economyset.command(pass_context=True)
    async def rarecredits(self, ctx, credits: int):
        """Credits earned by Rare Role members each payday"""
        server = ctx.message.server
        self.settings[server.id]["RARE_CREDITS"] = credits
        await self.bot.say("Every payday will now give {} credits."
                           "".format(credits))
        dataIO.save_json(self.file_path, self.settings)

    @economyset.command(pass_context=True)
    async def epiccredits(self, ctx, credits: int):
        """Credits earned by Epic Role members each payday"""
        server = ctx.message.server
        self.settings[server.id]["EPIC_CREDITS"] = credits
        await self.bot.say("Every payday will now give {} credits."
                           "".format(credits))
        dataIO.save_json(self.file_path, self.settings)

    @economyset.command(pass_context=True)
    async def legendarycredits(self, ctx, credits: int):
        """Credits earned by Legendary Role members each payday"""
        server = ctx.message.server
        self.settings[server.id]["LEGENDARY_CREDITS"] = credits
        await self.bot.say("Every payday will now give {} credits."
                           "".format(credits))
        dataIO.save_json(self.file_path, self.settings)
