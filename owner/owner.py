...
class Owner:
    ...
    @_set.group(name="coowner", pass_context=True)
    @checks.is__owner()
    async def co_owner(self, ctx):
        """Change co-owners"""
        if str(ctx.invoked_subcommand) == "set coowner":
            await self.bot.send_cmd_help(ctx)
            return

    @co_owner.command(pass_context=True)
    @checks.is__owner()
    async def add(self, ctx, user: discord.User):
        ...
        if self.bot.settings.no_prompt is True:
            ...
            if user.id not in self.bot.settings.co_owners:
                await self.bot.say(
                "{}, A co-owner has same perms as an owner. Setting owner to someone who"
                "does not host the bot. This should not be used improperly. It has"
                "repercussions and is *NOT recommended to set someone anonymous*. "
                "Proceed at your own risk.".format(ctx.message.author.mention)
                )
                await asyncio.sleep(3)

                await self.bot.say("Confirm in the console that you're the owner.")
                t = threading.Thread(target=self._wait_for_add_confirm,
                                     args=(user,))
                t.start()
            else:
    ...
    async def remove(self, ctx, user: discord.User):
        ...
        if self.bot.settings.no_prompt is True:
            ...
            if user.id in self.bot.settings.co_owners:
                await self.bot.say(
                    "{}, Are you sure you want to remove {} from co-owner?".format(user.name, ctx.message.author.mention)
                )
                await asyncio.sleep( 3 )

                await self.bot.say( "Confirm in the console that you're the owner." )
                t = threading.Thread( target=self._wait_for_del_confirm,
                                      args=(user,))
                t.start()
            else:
    ...
    def _wait_for_add_confirm(self, author):
        print(author.name + " has been prompted to be added as co-owner. If this is you, "
              "type 'yes'. Otherwise press enter.")
        print()
        print("*DO NOT* set anyone anonymous as co-owner. This has security "
              "repercussions.")
        ...
        if choice == "yes":
            self.bot.settings.co_owners.append(author.id)
            self.bot.settings.save_settings()
            print(author.name + " has been been added as co-owner.")
        else:
            print("The add co-owner request has been ignored.")

    def _wait_for_del_confirm(self, author):
        print(author.name + " has been prompted to be removed from co-owner. If this is you, "
              "type 'yes'. Otherwise press enter.")
        print()
        ...
        if choice == "yes":
            self.bot.settings.co_owners.remove(author.id)
            self.bot.settings.save_settings()
            print(author.name + " has been removed from co-owner.")
        else:
            print("The remove co-owner request has been ignored.")
